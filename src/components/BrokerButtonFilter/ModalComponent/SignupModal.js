...
const SignupModal = props => {
  ...
  const label = (
    <p className='white-text mb-0'>
      I agree to all the{' '}
      <span className={styles['link-text']}>Terms & Condition</span>
      and <span className={styles['link-text']}>Privacy policy</span>
    </p>
  );
  ...
  const handleClick = async () => {
    try {
      ...
      if (res?.status === 200) {
        ...
        dispatch(loginSignupAction.uploadSignup({ mobileNumber: contact, termCondition: checked }));
